...
struct Game{ 
    ...
    mutating func startGame(){
        ...
        for i in 0..<hight{
            var row:[Grid] = []
            for j in 0..<width{
                row.append(Grid(gridState: gridStatus, h: i, w: j))
            }
            board.append(row)
        }
        inputBoats()
                ...
                if directions == 0{
                    ...
                    if isEmpty(grid: board[hY][wX], directions: directions, boat:i){
                        for w in wX..<wX+i.boatNumber(){
                            board[hY][w] = Grid(gridState:GridState.destroyBoat, h: hY, w: w)
                        }
                        flag = false
                    } 
                    ...
                    if isEmpty(grid: board[hY][wX], directions: directions, boat: i){
                        for h in hY..<hY+i.boatNumber(){
                            ...
                            board[h][wX] = Grid(gridState:GridState.afterAttac, h: h, w: wX)
