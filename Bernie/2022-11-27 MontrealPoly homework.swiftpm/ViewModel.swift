...
struct ViewModel{
    ...
    func fillBGColor(_ x: Int, _ y:Int) -> Color {
        if y==0 && x>0 && x<10{
            return lands[0][x-1].backgroundColor ?? .clear
        }
        if x==0 && y>0 && y<10{
            return lands[1][y-1].backgroundColor ?? .clear
        }
        if x==10 && y>0 && y<10{
            return lands[2][y-1].backgroundColor ?? .clear
        }
        if y==10 && x>0 && x<10{
            return lands[3][x-1].backgroundColor ?? .clear
        }
        return .clear
    }
