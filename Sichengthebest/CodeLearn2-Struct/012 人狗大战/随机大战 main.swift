...
while animalToChoose.count > 0 || dogToChooseFrom.count > 0 {
    ...
    if animalToChoose.count > 1 || dogToChooseFrom.count > 1 {
        randomNumber -= randomNumber
        randomNumber += Int.random(in: 0...dogToChooseFrom.count-1)
        randomNess -= randomNess
        randomNess += Int.random(in: 0...animalToChoose.count-1)
    } else if animalToChoose.count < 2 || dogToChooseFrom.count < 2 {
        if animalToChoose.count == 1 {
            randomNess = 0
        } 
        if dogToChooseFrom.count == 1 {
            randomNumber = 0
        } else if animalToChoose.count == 0 {
            show("The dogs win!")
